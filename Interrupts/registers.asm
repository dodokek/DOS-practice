...
Start:                                   
		...
		; Hanging 08 interrupt

			mov ax, 3508h                       ; With this set of commands we find place 
			int 21h                             ; of the old 08h handler

			mov word ptr int08h_ptr, bx         ; saving adress of old handler
			mov word ptr int08h_ptr + 2, es     ;

			mov ax, 2508h                       ;
			mov dx, offset New08          		; changing int table with my own function
			int 21h                             ;

		; Hanging 09 interrupt
		
			mov ax, 3509h                       ; With this set of commands we find place
			int 21h                             ; of the old 09h handler

			mov word ptr int09h_ptr, bx         ; saving adress of old handler
			mov word ptr int09h_ptr + 2, es     ;

			mov ax, 2509h                       ;
			mov dx, offset New09	            ; changing int table with my own function
			int 21h                             ;
...
PrintHex		proc
				...
				mov di, hex_len * 2d 	; calculating len
				add di, dx
				std 

				push ax cx dx 			; saving registers
